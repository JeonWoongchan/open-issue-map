...
// 오버레이, 헤더, 좌우 2단 그리드(데모 카드 + 가이드 목록)를 통합 관리한다.
// 트리거 버튼은 없다 — 열림 상태는 호출부가 useHelpDialog로 들고 있다가 화면 우측 하단
// 공통 도움말 버튼(HelpFloatingButton)에 등록해서 연다(useRegisterFloatingHelp 참고).
...
type HelpDialogFrameProps<TGuideId extends string> = {
  ...
  onActivateGuideAction: (guideId: TGuideId) => void
  onClearGuideAction: () => void
  ...
  footer?: ReactNode
  // render prop — 위 activeGuideId/demoUpdatedAt 등을 데모 카드에 직접 전달하기 위해
  // ReactNode 대신 함수로 받음
          ...
          <div className="flex min-h-0 flex-1 flex-col overflow-hidden px-5 py-5 sm:px-6 lg:grid lg:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)] lg:gap-6">
            <div className="mb-4 shrink-0 lg:mb-0 lg:min-h-0 lg:overflow-y-auto lg:overscroll-contain lg:scrollbar-hide">
              {renderDemoCardAction({
                activeGuideId,
                demoUpdatedAt,
                onActivateGuide: onActivateGuideAction,
                onClearGuide: onClearGuideAction,
              })}
            </div>
