    ...
    return (
        <div className="space-y-5">
            ...
            <HelpSection number={1} title="이슈 찾기 & 클레임">
                <HelpDataTable>
                            <HelpTableRow label="이슈 탐색" description="Issues 탭에서 good first issue 라벨이 붙은 이슈를 찾는다." />
                            <HelpTableRow label="담당자 지정" description="이슈 댓글에 작업 의사를 남겨 관리자에게 assignee 지정을 요청한다." />
                </HelpDataTable>
            </HelpSection>
            ...
            <HelpSection number={2} title="레포 준비">
                <HelpDataTable>
                            <HelpTableRow label="Fork" description="원본 레포를 본인 GitHub 계정으로 Fork한다." />
                            <HelpTableRow label="Clone & remote" description="Fork 레포를 클론하고, 원본 레포를 upstream으로 등록한다." />
                            <HelpTableRow label="브랜치 생성" description="main에 직접 커밋하지 않고 feat/<작업내용> 브랜치에서 작업한다." />
                </HelpDataTable>
            </HelpSection>
            ...
            <HelpSection number={4} title="커밋 & PR">
                <HelpDataTable>
                            <HelpTableRow label="코드 스타일" description="기존 코드의 들여쓰기·따옴표 방식을 그대로 맞춘다." />
                            <HelpTableRow label="커밋 형식" description="Conventional Commits 형식 사용 — feat / fix / docs / style / chore 등" />
                            <HelpTableRow label="검증" description="npm run check 또는 레포에서 안내하는 명령으로 타입·린트 오류를 확인한다." />
                            <HelpTableRow label="PR 생성" description="base를 원본 레포 main으로 설정하고, Closes #이슈번호로 이슈를 연결한다." />
                </HelpDataTable>
            </HelpSection>
